...
分组表、示范和配对机器人文件参与训练指纹；目的采样有独立 RNG，保存并精确续训。G 不加载 task 的语言文件，允许省略其 language 字段；π 的训练语言契约保持不变。G 的 checkpoint/export 为 v4，明确拒绝旧 G v3 权重静默续训；阶段 D 将 π 工件升为 v4，数据及观测仍为 v3，E 身份没有因为新增意图头而改变。
