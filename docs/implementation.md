...
本轮按用户选择，先完成轻量作用编码器与现有读取器接入；视频损失不直接训练WAM。

| 提交阶段 | 内容 |
|---|---|
| a0992e4 | 单视角数据与监督，可靠同步视角CV改为可选 |
| 0be428d | 独立video_pretrain契约、连续作用瓶颈与多时域特征／部分作用监督 |
| 7f2c609 | 预训练、恢复、留出诊断、tokens导出、来源闭包审计与原始视频入口 |
| 文档收尾 | 固定用户选择、服务器资源占位、200项回归及原生单视角三阶段记录 |

保持固定候选协议、PhysicalOutcome/EffectRequirement分离和原生Zero-WAM源码版本。原有成对损失仍可使用，但不会要求每段网络视频拥有同步相机或机器人标签。
