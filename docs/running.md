...
.venv/bin/etude doctor
.venv/bin/etude check
```

源码检查也可用 `PYTHONPATH=src .venv/bin/python -m etude ...`。标准库 unittest 不另需测试框架。没有 CUDA 或原生依赖时，native 测试明确跳过；这不算 native 验证成功。
...
```bash
.venv/bin/etude validate-data --index /absolute/dataset/index.json
.venv/bin/etude make-fixture --output outputs/fixture
```
...
```bash
.venv/bin/etude train --config configs/V1.json --index outputs/fixture/index.json --tiny-native --stage interface --steps 1 --output outputs/interface
.venv/bin/etude train --config configs/V1.json --index outputs/fixture/index.json --tiny-native --stage reader --initialize outputs/interface/adapter.pt --steps 1 --output outputs/reader
.venv/bin/etude train --config configs/V1.json --index outputs/fixture/index.json --tiny-native --stage joint --initialize outputs/reader/adapter.pt --steps 1 --output outputs/joint
```
...
```bash
.venv/bin/etude train --config /absolute/audited-config.json --index /absolute/dataset/index.json --checkpoint /absolute/zero-wam-pretrain --stage interface --steps 1000 --output outputs/interface-real
```
...
```bash
.venv/bin/etude train --config configs/V1.json --index outputs/fixture/index.json --tiny-native --stage interface --resume outputs/interface/adapter.pt --steps 1 --output outputs/interface
```
...
```bash
.venv/bin/etude calibrate-f --artifact outputs/joint/adapter.pt --index /absolute/candidate-data/index.json --steps 100 --output outputs/calibrated/adapter.pt
.venv/bin/etude predict --artifact outputs/joint/adapter.pt --manifest outputs/fixture/observation.json --candidates 1 --diagnostic --output outputs/prediction
```

`calibrate-f` 只更新 F，记录数据窗口和实际交互量，不反复开启新一轮适配，也不校准成功概率。四候选预测要求已完成这一有限适配：

```bash
.venv/bin/etude predict --artifact outputs/calibrated/adapter.pt --manifest /absolute/evaluation/observation.json --checkpoint /absolute/zero-wam-pretrain --candidates 4 --scoring-config /absolute/validation/scoring.json --output outputs/candidates
```
