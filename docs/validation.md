...
- 第二阶段冻结已训练动作侧与目标／状态接入参数；动作损失本身能传回视觉目标读取器及视频LoRA，Pose Decoder产生有效SE(3)。视频／IFP目标继续单独训练WAM。
- 固定示范、真实历史与随机种子，替换真值目标、动作及真实未来画面，生成未来和接口tokens保持一致。动作专家对已填充或被篡改的视频K/V缓存均不敏感，只读取连续目标接口与当前状态。
- 两阶段各自连续训练与断点续训的模型、优化器和RNG一致；阶段转换重新建立优化器。目标坐标系、来源类型、state/action归一化及控制节拍不匹配时拒绝恢复或推理。
- 数据检查覆盖实测终点／控制器目标区别、有效SO(3)、逐动作块时域、每个未来视频时刻与控制节拍的一致性，以及源数据划分。已知同源机器人参考被拒绝，避免把目标真实未来放进参考入口；人类语义桥接仍可使用。
- 导出保留完整目标接口及视频／动作两侧适配器，跨进程CLI导出和仅观测输入的预测通过。缺失适配器、损坏策略、未来／动作／目标标签进入推理输入均被拒绝，推理权重保持冻结。
- 默认候选64×768接口在随机tiny-native底座上实际跑通 `goal → visual → export-goal-policy → predict-goal-policy`；两阶段各1次成功更新。输出动作 `[1,3,2,2,1]`、目标位姿 `[1,1,4,4]` 及生成未来均有限，未激活动作通道为零，commands_sent=0。
